...
class MyGetupEnv(Getup):
    def step(self, state: mjx_env.State, action: jax.Array):
        ...
        # print(f"body_lin_vel: {body_lin_vel.shape}")
        # print(f"body_ang_vel: {body_ang_vel.shape}")
        # print(f"gravity_vector: {gravity_vector.shape}")
        # print(f"default_qpos: {default_qpos.shape}")
        # print(f"joint_qpos: {joint_qpos.shape}")
        ...
        # jax.debug.print("body_pos: {}", body_pos)
        height_reward = jp.exp(-5 * (DESIRED_BODY_HEIGHT - body_pos[2])**2)

        stability_reward = 0.5 * jp.exp(-0.1 * (jp.linalg.norm(body_lin_vel) + jp.linalg.norm(body_ang_vel)))

        joint_pos_reward = jp.exp(-2 * jp.linalg.norm(joint_qpos - default_qpos))
        joint_vel_penalty = 0.1 * jp.linalg.norm(joint_qvel)

        orientation_reward = -gravity_vector[2]
